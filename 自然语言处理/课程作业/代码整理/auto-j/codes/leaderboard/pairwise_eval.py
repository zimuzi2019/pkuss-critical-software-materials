...
def exchange_to_ori_label(exchanged_pred_label):
    assert exchanged_pred_label in [0, 1, 2, -1]
...
def check_res(gt_label, pred_label, pred_label_exchange, ):
    assert pred_label in [0, 1, 2, -1] and pred_label_exchange in [0, 1, 2, None, -1]
    correct = [0, 0]
    agree = 0
    both_correct = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pred_file_path", type=str, default="/home2/xjw/prediction.jsonl", help="path to the prediction file")
    parser.add_argument("--exchange_pred_file_path", type=str, default="/home2/xjw/prediction_exchange.jsonl", help="path to the prediction file on the `exchange-response-order` data")
    parser.add_argument("--source_file_path", type=str, default="/home2/xjw/auto-j/data/test/testdata_pairwise.jsonl", help="path to the source file")
    #parser.add_argument("--type", type=str, default="pairwise", choices=["single", "pairwise"], help="single or pairwise")
    #parser.add_argument("--pred_file_path", type=str, default="/home2/xjw/auto-j/data/outputs/pairwise_example_output.jsonl", help="path to the prediction file")
    #parser.add_argument("--exchange_pred_file_path", type=str, default="/home2/xjw/auto-j/data/outputs/pairwise_exchange_example_output.jsonl", help="path to the prediction file on the `exchange-response-order` data")
    #parser.add_argument("--source_file_path", type=str, default="/home2/xjw/auto-j/data/test/testdata_pairwise.jsonl", help="path to the source file")
